...
# wget https://bakery.ton.swisscops.com/dumps/latest.tar.lz -P /var/ton-work/db
# tar --lzip -xvzf latest.tar.lz -C /var/ton-work/db
...
if [ ! -z "$GCONFURL" ]; then
    ...
    wget -q https://ton.org/global-config.json -O /var/ton-work/db/ton-global.config
fi
